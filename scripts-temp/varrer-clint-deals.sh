#!/bin/bash
# Varre todos deals do Clint via clint-mapear-deals (~125k deals em ~630 paginas)
set -a; source c:/Squad/.env.local 2>/dev/null; set +a
URL="https://${SUPABASE_PROJECT_REF:-wmelierxzpjamiofeemh}.supabase.co/functions/v1/clint-mapear-deals"
PAGE=1
ONDAS=0
FALHAS_SEGUIDAS=0
...
while true; do
  ...
  DEALS=$(node -e "try{const d=JSON.parse(process.argv[1]);console.log(d.deals_lidos||0)}catch(e){console.log(0)}" "$RESP" 2>/dev/null)
  ...
  echo "  paginas=$PAGINAS deals=$DEALS produtos=$PRODUTOS ultima_proc=$ULTIMA_PROC tem_mais=$TEM_MAIS proxima=$PROXIMA"

  if [ "$PROXIMA" = "parse-error" ]; then
    FALHAS_SEGUIDAS=$((FALHAS_SEGUIDAS+1))
    echo "  Falha de rede. Tentativa $FALHAS_SEGUIDAS/3."
    if [ $FALHAS_SEGUIDAS -ge 3 ]; then echo "  3 falhas. Abortando."; break; fi
    sleep 10
    continue
  fi
  FALHAS_SEGUIDAS=0
  ...
  PAGE=$PROXIMA
  if [ $ONDAS -gt 50 ]; then echo "Limite 50 ondas atingido."; break; fi
done
echo "DONE"
