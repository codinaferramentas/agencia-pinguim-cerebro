...
**⚠ DESCOBERTA REVISADA 2026-05-11 noite finalíssima — API Hotmart expõe 7+ Clubs:**

Investigação corrigida: a URL admin web `/membership/turbo-x/products/X` é só PATH, NÃO subdomain API. **Subdomain API real é o nome do produto sem hífens/lowercase.**

**Clubs ACESSÍVEIS via API (validado 2026-05-11):**

| Subdomain | Produto | Alunos | Produto ID |
|---|---|---|---|
| `proalt` | ProAlt - Low Ticket | 478 | 6811692 |
| `proaltlowticketrecorrente` | ProAlt - Low Ticket Recorrente | 42 | 6814308 |
| `siriuslab` | Sirius Lab (⚠ NÃO é o Sirius app — ver abaixo) | 838 | 5863082 |
| `protocolovendaviral` | Protocolo Venda Viral | 450 | 7189437 |
| `mastermindorion` | Mastermind Orion | 32 | 2634662 |
| `taurusmentoring` | Taurus Mentoring | 1 | 2605400 |
| `escoladoperpetuo` | Escola do Perpétuo | 1 | 1784595 |

**Pendentes (Andre precisa descobrir subdomain real):** Elo (6404411), Lyra (4044671), Desafio Low Ticket (6662699), 365 Roteiros (5984739), 120 Sequências (6322358), Análise Perfil (5673093), Lo-Fi/Edição Conteúdo (5984961), Turbo X (3594811), Imersão (7395100).

**Padrão de subdomain descoberto:** nome do produto em lowercase, sem espaços e sem hífens (ex: "Mastermind Orion" → `mastermindorion`). Mas nem todos seguem — `proalt` (não `proaltlowticket`). **Sempre validar empiricamente.**

**Pra consultar acesso de aluno:** rodar `bash scripts/hotmart-verificar-acesso-membros.sh <email>` que itera nos Clubs cadastrados em `pinguim.hotmart_clubs` (ativo=true) e retorna onde achou.

**⚠ REGRA DE NEGÓCIO CRÍTICA — Sirius vs Sirius Lab (Andre 2026-05-11):**

Existe DIFERENÇA entre:
- **Sirius (app)** — sistema/produto da Pinguim. Tem **mais de 20.000 alunos**. Dados ficam em **Supabase próprio** (`SIRIUS_SUPABASE_URL`/`KEY` no cofre). **Pra consultar Sirius app, USAR Categoria M (Supabase externo), NUNCA Hotmart.**
- **Sirius Lab (Hotmart product 5863082)** — produto de curso vendido na Hotmart. 838 alunos no Club `siriuslab`. Provavelmente compradores de tokens/créditos do app Sirius. **NÃO confundir com o Sirius app**.

**Regra dura:** quando sócio falar "Sirius", "alunos do Sirius", "consultar Sirius", **PERGUNTAR primeiro** se é o app Sirius (Supabase) ou o curso Sirius Lab (Hotmart). Quando estiver claro pelo contexto (sócio menciona "20 mil alunos" → app; "comprou Sirius Lab" → curso), usar fonte correta. **Default: assumir app Sirius (Supabase)** quando ambíguo, porque é o caso de uso principal.

**Anti-padrão:** dizer "esse aluno está ativo no Sirius com 47 aulas concluídas" baseado em consulta no Club `siriuslab` Hotmart quando sócio queria saber do **app**. Confunde dado e quebra confiança.
